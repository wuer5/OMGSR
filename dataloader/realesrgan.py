...
from torchvision.transforms.functional import (adjust_brightness, adjust_contrast, adjust_hue, adjust_saturation, rgb_to_grayscale)
...
class RealESRGAN_degradation(object):
    ...
    def random_kernels(self):
        ...
        if np.random.uniform() < self.sinc_prob:
            # this sinc filter setting is for kernels ranging from 512: [7, 21] or 1024: [11, 41]
            if kernel_size < self.kernel_mid:
            ...
            kernel = random_mixed_kernels(
                    ...
                    noise_range=None)
        # pad kernel
        pad_size = (self.kernel_max - kernel_size) // 2
        ...
        if np.random.uniform() < self.final_sinc_prob:
            kernel_size = random.choice(self.kernel_range)
            omega_c = np.random.uniform(np.pi / 3, np.pi)
            sinc_kernel = circular_lowpass_kernel(omega_c, kernel_size, pad_to=self.kernel_max)
